...
class SaleShop(models.Model):
            ...
            try:
                ...
                if 'country' in prestashop_country_data.get('countries'):
                    ...
                    for country in country_list:
                        ...
                        if not country_id:
                            country_vals.update({'name': country.get('name').get('language').get('value'),
    ...
    def manageOrderLines(self, orderid, order_detail, prestashop):
        ...
        if order_rows is not None:
            ...
            for child in order_rows:
                ...
                if self.get_value_data(child.get('product_attribute_id')) != '0':
                    ...
                    temp_id = prod_templ_obj.search([('presta_id', '=', self.get_value_data(child.get('product_id'))),
                    ...
                    if temp_id:
                        ...
                        if product_ids:
                            ...
                            # product_ids = product_obj.search([('product_tmpl_id.presta_id', '=', self.get_value_data(child.get('product_attribute_id')))])
                            product_ids = product_obj.search(
                                [('default_code', '=', self.get_value_data(child.get('product_reference')))], limit=1)
                ...
                if 'product_id' not in line:
                    ...
                    if product_id:
                        group_tax_id = product_id.tax_group_id
                        line.update({'product_id': product_id[0].id, 'product_uom': product_id[0].uom_id.id})
                    else:
                        product_id = self.remove_record_prestashop_checked(prod_templ_obj, 'Removed Product',
                                                                           {'name': 'Removed Product'})
                        line.update({'product_id': product_id.id, 'product_uom': product_id.uom_id.id})
        ...
        discoun = float(order_detail.get('total_discounts'))
        if discoun and discoun > 0:
            ...
            dline_ids = sale_order_line_obj.search(
                [('product_id', '=', self.get_value_data(discount_line.get('product_id'))),
                 ('order_id', '=', orderid.id)])
            if not dline_ids:
    ...
    def create_presta_order(self, order_detail, prestashop):
        ...
        try:
            ...
            order_vals.update({
                ...
                'pretsa_payment_mode1': order_detail.get('payment') or False,
        ...
        try:
            for shop in self:
                ...
                filters = {'display': 'full', 'filter[id]': '>[%s]' % shop.last_order_id_id_import,
                           'filter[id_shop]': '%s' % self.presta_id, 'limit': 50}
        ...
        try:
            ...
            for shop in self:
                prestashop = PrestaShopWebServiceDict(shop.shop_physical_url,
                                                      shop.prestashop_instance_id.webservice_key or None)
                # prestashop = PrestaShopWebServiceDict(shop.prestashop_instance_id.location,shop.prestashop_instance_id.webservice_key or None)
                filters = {'display': 'full', 'filter[id]': '=[%s]' % shop.last_cart_rule_id_import,
            ...
            try:
                ...
                for order in order_ids:
                    ...
                    order_schema.get('order').update({
                        ...
                        'id_address_invoice': order.partner_invoice_id.address_id and str(
                            order.partner_invoice_id.address_id),
